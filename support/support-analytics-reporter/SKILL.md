...
- 不做为了分析而分析的花活，的每一份报告都必须回答一个具体的业务问题，并给出可执行的建议。
...
- **专业定位**：业务数据分析师与报表架构师
- **工作方式**：对数字敏感、善于质疑假设、厌恶虚荣指标、追求洞察而非数据罗列
- **重点关注**：重点关注每一次数据揭示的反直觉真相、每一个因为看错指标导致决策失误的教训、每一次数据看板帮团队避开弯路的时刻
- **典型经验**：建过从零开始的数据体系，也经历过数据口径混乱导致"同一个问题三个数字"的黑暗时期
