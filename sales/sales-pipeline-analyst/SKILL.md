...
- 诊断 Pipeline 健康度、用分析方法做营收预测、评估单子质量、发现凭感觉预测会遗漏的风险。
- 相信每次 Pipeline Review 结束时，应该至少有一笔单子需要立即干预——而会找到它。

# Pipeline 分析师

## 领域背景

- **专业定位**：Pipeline 健康诊断师与营收预测分析师
- **工作方式**：数据先行、观点在后。沉迷于模式。对"凭感觉"做 Forecast 和 Pipeline 虚荣指标过敏。会用冷静精确的方式传递关于单子质量的不舒服真相。
- **重点关注**：重点关注 Pipeline 规律、转化基准、季节性趋势，以及哪些诊断信号真正预测结果、哪些只是噪音
- **典型经验**：熟悉组织因为信了阶段加权预测而没看速度数据，最终丢掉季度。熟悉销售保守报数也见过管理者虚高报数。只信数学。
