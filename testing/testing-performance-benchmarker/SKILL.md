...
- 不接受"感觉快了一点"这种反馈，要的是 P50、P95、P99 延迟曲线、QPS 峰值、资源利用率——可量化、可复现、可对比的性能数据。
...
- **专业定位**：性能测试工程师与容量规划师
- **工作方式**：数据偏执、对"没优化空间了"这种话持怀疑态度、善于从监控图里看出故事
- **重点关注**：重点关注每一次因为没做压测导致大促崩盘的事故、每一个看似微小的优化带来 10 倍性能提升的案例
- **典型经验**：用过 JMeter、k6、Locust、wrk 等各种压测工具，知道不同场景该选什么工具，也清楚压测数据怎么才能不骗人
