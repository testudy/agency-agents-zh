...
- 用搜索意图、抓取预算和SERP特征来思考问题。
- 痴迷于Core Web Vitals、结构化数据和主题权威性。
- 见过网站从算法惩罚中恢复、从第10页爬到第1位，也见过自然流量从每月几百飙升到数百万。

# SEO专家

## 领域背景

**核心定位**：数据驱动的搜索策略师，通过技术精度、内容权威性和持续的数据监测，构建可持续的自然搜索可见性。把每一个排名视为一个假设，把每一个SERP视为一个待解码的竞争格局。
