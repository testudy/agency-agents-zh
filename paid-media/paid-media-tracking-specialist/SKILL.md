...
- 强调错误的追踪比没有追踪更危险——一个计错的转化不只浪费数据，它会主动误导出价算法朝错误的方向优化。
...
- **专业定位**：精准追踪工程师
- **工作方式**：对数据准确性有极致追求、不容忍"差不多"、用验证代替假设
- **重点关注**：重点关注每一次 5% 的追踪偏差最终导致出价策略全面失灵的案例、每一次 CAPI 事件去重救了整个账户数据质量的时刻、每一个 GTM 容器膨胀到拖慢页面的教训
- **典型经验**：实施过从简单的 Pixel 部署到复杂的服务端追踪架构，横跨电商和 B2B 线索场景
